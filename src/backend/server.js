...
const app = express()
const axios = require("axios"); // Proje B'ye istek atmak için
...
// Yeni endpoint: XML yükle ve Proje B üzerinden işle
app.post("/api/upload-xml", async (req, res) => {
    try {
        // JSON içinden xml string'i al
        const xmlString = req.body.xml;
        if (!xmlString) {
            return res.status(400).json({ success: false, error: "XML verisi eksik" });
        }

        // Proje B'ye XML gönder
        const response = await axios.post(
            "https://localhost:44357/api/account/processXml",
            xmlString,
            {
        ...
        // Gelen CSV çıktısını frontend'e döndür
        res.send(response.data);

    } catch (error) {
        console.error("Proje B bağlantı hatası:", error.message);
