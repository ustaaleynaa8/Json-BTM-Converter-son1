...
import { XmlBtmService } from "./services/xml-btm.service"   // ✅ Yeni servis import
...
export class AppComponent {
    ...
    constructor(
        ...
        private xmlBtmService: XmlBtmService  // ✅ Yeni servis inject
    ) { }
    ...
    /**
     * XML dosyası seçildiğinde çalışan method
    ...
    onXmlSelected(event: any): void {
        ...
        if (file) {
            ...
            reader.onload = () => {
                ...
                // 1️⃣ XML'i B projesine gönder
                this.xmlBtmService.uploadXml(xmlString).subscribe({
                    next: (csvString: string) => {
                        console.log("B projesinden gelen CSV:", csvString);
                        const jsonArray = this.parseCsvToJson(csvString);
                        this.jsonData = jsonArray;
                        this.tableHeaders = Object.keys(jsonArray[0] || {});
                        alert("XML → BTM → CSV → JSON akışı başarılı!");
                    },
                    error: (err) => {
                        console.error("BTM servis hatası:", err);
                        alert("BTM servisinde hata oluştu");
                    }
                });
